...
class BackOffice extends Component {
  ...
  render() {
        ...
        <form>
          ...
          >
            ...
            {this.props.characters.map((char, index) => (
              <option key={char.id} value={index}>
                {char.name}
              </option>
            ))}
          </select>
          ...
          <Button
            ...
            clicked={event => this.submitCharacter(event)}
          />
        </form>
      );
    } else {
    ...
    return (
      <Aux>
        <section className={Classes.BackOffice}>{backInput}</section>
      </Aux>
    );
  }
}
...
const mapDispacthToProps = dispatch => {
  return {
    onLoadCharacters: token =>
      dispatch(actionsCreators.getBackCharacters(token)),
    onChangeStatus: (index, status, token) =>
      dispatch(actionsCreators.patchCharacterStatus(index, status, token))
  };
};
